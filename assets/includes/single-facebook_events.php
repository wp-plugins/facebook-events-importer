...
$fbe800 = get_option("facebook_events_pro_version");  if($fbe800){  }else{ exit; }
   while (have_posts()) :
      the_post();
		  ...
		  $event_start_date = get_fbe_date('event_starts','M j, Y',$timezone);
		  $event_start_time = get_fbe_date('start_time','g:i a',$timezone);
		  $event_end_date = get_fbe_date('event_ends','M j, Y',$timezone);
		  $event_end_time = get_fbe_date('end_time','g:ia',$timezone);
