   ...
   if( $loop->have_posts() ){ 
	 ...
	 $post_information = array(
		...
		*/
        ...
        $startDate = gmstrftime($startDate);
		$start_time = date('g:i a',$startDate);
		$start_date = date('m/d/Y',$startDate);

		update_post_meta($post_id,'start_time', $start_time);
		update_post_meta($post_id,'event_starts_formatted', $start_date);
        
        }else{
	    update_post_meta($post_id,'start_time', '');
		update_post_meta($post_id,'event_starts_formatted','');
	    }
		...
		$end_time = date('g:i a',$endDate);
		$end_date = date('m/d/Y',$endDate);

		update_post_meta($post_id,'event_ends_formatted', $end_date);
		update_post_meta($post_id,'end_time', $end_time);

	    }else{
	    update_post_meta($post_id,'event_ends_formatted', '');
