...
try {
  ...
  echo $ex->getMessage();
	...
	$eventResponse = (new FacebookRequest($session, 'GET', '/'.$facebook_page.'/events?fields=venue,location,cover,attending_count,description,end_time,id,name,owner,start_time,ticket_uri,timezone,is_date_only'))->execute()->getResponse();
	...
	foreach ($events as $e) {	
		...
		$location = wp_strip_all_tags($e->location);
   ...
   if( $loop->have_posts() ){ 
	...
	echo '<div class="error" style="color:#222222; font-weight:700; font-size:1em; padding:10px">There may not be any future events or the page is age restricted.</div>';
	}
	if($a >= 1){
	...
	echo '<div class="error" style="color:#222222; font-weight:700; font-size:1em; padding:10px">There are '.$c . ' events for this page</div>';
	}else{
