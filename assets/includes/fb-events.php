...
function create_fbe_post_event() {
  ...
  register_post_type( 'facebook_events',
    array(
      ...
      'public' => true,
      'menu_position' => 5, 
      'menu_icon' =>  plugins_url('admin_icon.png', __FILE__ ), 
...
function fbe_meta_box_callback( $post ) {
  ...
  $event_starts = get_post_meta( $post->ID, 'event_starts', true );
  $event_ends = get_post_meta( $post->ID, 'event_ends', true );
  ...
  $event_starts_formatted = get_post_meta( $post->ID, 'event_starts_formatted', true );
  $event_ends_formatted = get_post_meta( $post->ID, 'event_ends_formatted', true );

  $from_time = get_post_meta( $post->ID, 'start_time', true );
  $end_time = get_post_meta( $post->ID, 'end_time', true );

 if($event_starts_formatted == "" && $event_starts != ""){
  $fbstartdate = strtotime($event_starts);
  $event_starts_formatted = date('m/d/Y', $fbstartdate);
  $from_time = date('g:i a', $fbstartdate);
  }
  
  if($event_ends_formatted == "" && $event_ends != ""){
  $fbenddate = strtotime($event_ends);
  ...
  echo '<input type="text" id="event_starts_formatted" name="event_starts_formatted" value="'. sanitize_text_field( $event_starts_formatted ) .'" size="10">@<input type="text" id="start_time" name="start_time" value="'. sanitize_text_field( $from_time ) .'"" size="8" />';
  echo '<br />';
  echo '<label for="event_ends_formatted">Event Ends</label>';
  echo '<input type="text" id="event_ends_formatted" name="event_ends_formatted" value="'. sanitize_text_field( $event_ends_formatted ) .'" size="10">@<input type="text" id="end_time" name="end_time" value="'. sanitize_text_field( $end_time ) .'"" size="8" />';
  echo '<br />';
  ...
  echo '<input type="hidden" id="event_ends" name="event_ends" value="' . sanitize_text_field( $event_ends ) . '" size="25" />';
  echo '<input type="hidden" id="event_starts" name="event_starts" value="' . sanitize_text_field( $event_starts ) . '" size="25" />';
  echo '</div>';
...
if( isset( $_POST[ 'feature_event' ] ) ) {
  ...
  $fields = "location,ticket_uri,event_starts,event_ends,facebook_event_id,image_url,start_time,end_time,event_starts_formatted,event_ends_formatted,fb_event_uri";
...
function get_fbe_date($meta,$format){
  ...
  $fbstartdate = strtotime($event_date);
  return date($format, $fbstartdate);
}
