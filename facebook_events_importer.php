 ...
 * Author URI: http://jasonprescher.com/
  * License: GPL2
 /*  Copyright 2015  Jason Prescher  (email : jason@volk.io)
...
update_option('facebook_events_free','installed');
  ...
  function import_fbe_scripts() {
	  ...
	  wp_enqueue_script('fbe', plugins_url( '/assets/js/facebook_import.js', __FILE__ ), array('jquery'), '1.0.0');		
  ...
  function fbe_remove_msg_callback(){	   
        $facebook_page = sanitize_text_field($_POST["page"]);
...
?>	
   ...
   <form id="wfei_plugin_settings" method="post" >
	...
	<p>Reload page events. Deleting does <b>not</b> remove events previously imported.</p>
	<ul>
	<?
...
add_action( 'wp_ajax_fbe_remove_msg', 'fbe_remove_msg_callback' );
add_action( 'wp_ajax_nopriv_fbe_remove_msg', 'fbe_remove_msg_callback' );
